...
@Component({
  ...
  templateUrl: './ecografiavenosa.component.html',
  styleUrl: './ecografiavenosa.component.css',
  providers: [ConfirmationService,MessageService]
})
export class EcografiavenosaComponent implements OnInit {
  ...
  ecografiavenosaForm = new FormGroup({
  ...
  buscarPacientes() {
    this.spinner = false;
    let documento = this.ecografiavenosaForm.get("dni_ecografia_mama")?.value;
    ...
    this.admisionServices
        ...
        .subscribe((response: any ) => {
          if(response.status == 200) {
            ...
            this.ecografiavenosaForm.patchValue({
